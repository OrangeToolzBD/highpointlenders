...
export const Route = createFileRoute("/austin/$suburb/$pillar")({
  head: ({ params }) => {
    const s = getSuburb(params.suburb);
    const p = getPillar(params.pillar);
    const title = s && p ? `${p.title} in ${s.name}, TX` : "Austin Loan Program";
    const description =
      s && p
        ? `${p.title} for ${s.name} businesses: ${p.tagline.toLowerCase()}. ${p.highlight}. Soft credit pull only.`
        : "Austin business funding programs.";
    const path = `/austin/${params.suburb}/${params.pillar}`;
    return buildHead({
      title,
      description,
      path,
        ...
        <section className="relative overflow-hidden border-b border-border/60">
          ...
          <div className="mx-auto max-w-4xl px-6 py-20">
            <div className="inline-flex items-center gap-2 rounded-full border border-border bg-card px-3 py-1 text-xs font-medium text-muted-foreground">
              ...
              {suburb.name}, TX · {suburb.county}
            </div>
            <h1 className="mt-5 text-4xl font-bold tracking-tight md:text-5xl">
              {pillar.title} in {suburb.name}, TX
            </h1>
            ...
            <p className="mt-5 text-base text-muted-foreground md:text-lg">
              ...
              that depend on Austin's year-round foot traffic, Texas tech inflow and the broader Central Texas economy.
            </p>

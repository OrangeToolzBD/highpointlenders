...
export const Route = createFileRoute("/austin/$suburb")({
  head: ({ params, matches, match }) => {
    if (matches[matches.length - 1]?.routeId !== match.routeId) return {};
    const s = getSuburb(params.suburb);
    const title = s ? `Business Loans in ${s.name}, TX` : "Austin Neighborhood";
    const description = s?.intro?.slice(0, 158) ?? "Austin neighborhood business funding.";
    const path = `/austin/${params.suburb}`;
    return buildHead({
      title,
      description,
      path,
  ...
  notFoundComponent: () => (
    <div className="mx-auto max-w-2xl px-6 py-32 text-center">
      ...
      <Button asChild className="mt-6"><Link to="/austin">Back to hub</Link></Button>
    </div>
  ),
      ...
      <main>
        <div className="border-b border-border/60 bg-card/30">
          <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-3 text-sm">
            <Link to="/austin" className="inline-flex items-center gap-2 text-muted-foreground hover:text-foreground">
              <ArrowLeft className="h-4 w-4" /> Austin hub
            </Link>
        ...
        <section className="relative overflow-hidden border-b border-border/60">
          ...
          <div className="mx-auto max-w-4xl px-6 py-20">
            ...
            <h1 className="mt-5 text-4xl font-bold tracking-tight md:text-5xl">
              Business Loans in {suburb.name}, TX
            </h1>
